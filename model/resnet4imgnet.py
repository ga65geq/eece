...
class ResNet(BasicModel):
    ...
    @staticmethod
    def _load_Bottleneck(bottleneck, pre_bottleneck):
        bottleneck.residual_function[0].load_state_dict(pre_bottleneck.conv1.state_dict())
        bottleneck.residual_function[1].load_state_dict(pre_bottleneck.bn1.state_dict())
        bottleneck.residual_function[3].load_state_dict(pre_bottleneck.conv2.state_dict())
        bottleneck.residual_function[4].load_state_dict(pre_bottleneck.bn2.state_dict())
        bottleneck.residual_function[6].load_state_dict(pre_bottleneck.conv3.state_dict())
        bottleneck.residual_function[7].load_state_dict(pre_bottleneck.bn3.state_dict())
        if pre_bottleneck.downsample is not None:
            assert (len(bottleneck.shortcut) != 0)
            bottleneck.shortcut[0].load_state_dict(pre_bottleneck.downsample[0].state_dict())
            bottleneck.shortcut[1].load_state_dict(pre_bottleneck.downsample[1].state_dict())

    def load_pretrained_model(self, pre_model):
        self.conv1[0].load_state_dict(pre_model.conv1.state_dict())
        self.conv1[1].load_state_dict(pre_model.bn1.state_dict())
        for i in range(self.num_block[0]):
            self._load_Bottleneck(self.conv2_x[i], pre_model.layer1[i])

        for i in range(self.num_block[1]):
            self._load_Bottleneck(self.conv3_x[i], pre_model.layer2[i])

        for i in range(self.num_block[2]):
            self._load_Bottleneck(self.conv4_x[i], pre_model.layer3[i])

        for i in range(self.num_block[3]):
            self._load_Bottleneck(self.conv5_x[i], pre_model.layer4[i])

        self.fc.load_state_dict(pre_model.fc.state_dict())
    ...
    def _normal_test_mac_forward(self, x):
        ...
        for layer in self.conv1:
            x = layer(x)
            if isinstance(layer, nn.Conv2d):
                self.statistic.conv_flops(x, 7, 3)
                self.statistic.conv_mac(x, 7, 3)
            elif isinstance(layer, nn.ReLU):
                self.statistic.relu_flops(x)
    ...
    def _snet_test_fine_tune_forward(self, x):
        ...
        for layer in self.conv1:
            ...
            if isinstance(layer, nn.Conv2d):
                self.statistic.conv_flops(x, 7, 3)
                tmp_mac = self.statistic.conv_mac(x, 7, 3)
            elif isinstance(layer, nn.ReLU):
                self.statistic.relu_flops(x)
        self._snet_mac(x, self.snet_body[0])
